...
def test_by_classification_caches_result() -> None:
    with patch("app.services.retention._repo.by_classification", return_value=_MOCK_ROWS) as mock:
        svc.by_classification(term="Fall2024")
        svc.by_classification(term="Fall2024")
    assert mock.call_count == 1


def test_by_classification_passes_filters() -> None:
    with patch("app.services.retention._repo.by_classification", return_value=_MOCK_ROWS) as mock:
        svc.by_classification(term="Fall2024", classifications=["Junior"])
